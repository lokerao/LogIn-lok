...
export default function TalentScreen() {
  ...
  const loadTalentData = useCallback(async (isRefresh = false) => {
    if (!session) {
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      const { data: empData, error: empError } = await supabase
        .from('employees')
        .select('id, employee_code, first_name, last_name, designations(name)')
        .eq('profile_id', session.user.id)
        .maybeSingle();

      if (empError) {
        setError('Could not load employee details. Please check your connection.');
        setIsLoading(false);
      ...
      const designationName =
        (empData.designations as unknown as { name?: string } | null)?.name ?? null;
      ...
      if (profileData) {
        const [
          ...
          supabase
            ...
            .select('id, employee_id, skill_id, proficiency, review_status, reviewer_notes, skills(id, name, category)')
            .eq('employee_id', empId)
        ...
        if (skillsRes.data) {
          const mappedSkills: EmployeeSkill[] = skillsRes.data.map((item) => {
            const skillObj = item.skills as unknown as { id: string; name: string; category: string | null } | null;
            return {
              id: item.id,
