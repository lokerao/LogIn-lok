...
import { fetchAdminHRLeaveRequests } from "@/features/leave/leave-service";
import type { LeaveRequest } from "@/types/leave";

type Filter = "pending" | "all";

export function AdminHRLeaveReview() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchAdminHRLeaveRequests(
        filter === "pending" ? "pending" : undefined,
      );
      setRequests(data);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Could not load HR leave requests.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <View style={employeeStyles.card}>
        <Text style={employeeStyles.label}>HR Personal Leave Requests</Text>
        <Text style={employeeStyles.muted}>
          Review and approve personal leave requests submitted by HR personnel.
        </Text>
        ...
        {!loading && !error && requests.length === 0 && (
          <Text style={employeeStyles.muted}>
            {filter === "pending"
              ? "No pending HR personal leave requests."
...
const styles = StyleSheet.create({
  ...
  errorBox: { gap: spacing.xs, marginTop: spacing.sm },
