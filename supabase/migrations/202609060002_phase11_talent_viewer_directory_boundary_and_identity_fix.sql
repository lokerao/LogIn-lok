-- ============================================================
-- Migration: 202609060002_phase11_talent_viewer_directory_boundary_and_identity_fix.sql
-- Description:
--   Phase 11 Talent Viewer & HR/Admin Directory Boundary Fix
--   1. Cleans up stale 'test recruiter' display names in public.profiles and public.employees.
--   2. Safely deletes/deactivates any accidental employee rows for talent_viewer accounts.
--   3. Updates get_hr_employee_directory() to strictly exclude talent_viewer profiles at the database level.
--   4. Updates get_hr_dashboard_summary() to exclude talent_viewer profiles from employee metrics.
--   5. Updates get_hr_employee_details() to protect against inspecting talent_viewer profiles as employees.
--   6. Updates get_admin_employee_directory() to exclude talent_viewer profiles from employee lists.
-- ============================================================

-- 1. CLEAN UP STALE TEST RECRUITER NAMES & SEED DATA
-- ============================================================
-- Update profiles display_name for talent viewers and test recruiters
update public.profiles
set display_name = 'Talent Viewer'
where id in (
  select profile_id from public.user_roles where role_key = 'talent_viewer'
)
or lower(trim(display_name)) in ('test recruiter', 'recruiter');

-- Update any accidental employee record names
update public.employees
set first_name = 'Talent',
    last_name = 'Viewer',
    employment_status = 'inactive'
where profile_id in (
  select profile_id from public.user_roles where role_key = 'talent_viewer'
)
or (lower(trim(first_name)) = 'test' and lower(trim(last_name)) in ('recruiter', 'viewer'));

-- Safely attempt to remove accidental employees row for external talent viewers
do $$
begin
  ...
  where profile_id in (
    ...
    where ur.role_key = 'talent_viewer'
  );
exception when foreign_key_violation then
  -- If foreign keys reference it (e.g. from prior tests), mark strictly inactive
  update public.employees
...
-- 2. GET HR EMPLOYEE DIRECTORY (STRICT TALENT VIEWER EXCLUSION)
-- ============================================================
...
-- 3. GET HR DASHBOARD SUMMARY (EXCLUDE TALENT VIEWERS)
-- ============================================================
...
begin
  ...
  -- Permanent employment status counts (excluding external talent viewers)
  select
    count(*),
...
-- 4. GET HR EMPLOYEE DETAILS (EXCLUDE TALENT VIEWERS)
-- ============================================================
...
-- 5. GET ADMIN EMPLOYEE DIRECTORY (EXCLUDE TALENT VIEWERS)
-- ============================================================
