-- ============================================================
-- Migration: 202609060004_fix_hr_employee_details_leave_balances.sql
-- Description:
--   Surgical fix for HR Hub / Organization Hub Employee Details RPC:
--   Resolves error "column lb.leave_type does not exist" in get_hr_employee_details(uuid).
--
--   Root cause:
--   In 202609060002 / 202609060003, get_hr_employee_details() was updated with
--   talent_viewer boundary checks, but accidentally used non-existent column
--   references on public.leave_balances ('lb.leave_type', 'lb.pending_days')
--   and public.leave_requests ('lr.leave_type', 'lr.total_days', 'lr.rejection_reason').
--
--   Fix:
--   Restores canonical Phase 4/9 relations:
--   - Joins public.leave_types lt on lt.id = lb.leave_type_id (projecting lt.name as leave_type_name)
--   - Joins public.leave_types lt on lt.id = lr.leave_type_id (projecting lt.name as leave_type_name, requested_days)
--   - Restores full canonical talent_profile sub-object and attendance_history keys matching frontend HREmployeeFullDetails
--   - Preserves all Phase 11 security boundaries (HR-only, org-scoped, talent_viewer inspection exclusion)
-- ============================================================
...
begin
  ...
  if not found or v_emp.id is null then
    raise exception 'Employee not found in your organization or access denied' using errcode = '42501';
  end if;
  ...
  -- 2. Leave balances for current year (correctly join leave_types on leave_type_id)
  select coalesce(jsonb_agg(
    jsonb_build_object(
  ...
  -- 3. Recent leave requests (last 10 records, correctly join leave_types on leave_type_id)
  select coalesce(jsonb_agg(
    jsonb_build_object(
      'id', lr.id,
      'leave_type_name', lt.name,
  ...
  -- 4. Recent work assignments (last 10 records)
  select coalesce(jsonb_agg(
    jsonb_build_object(
      'id', wk.id,
      'title', wk.title,
  ...
  -- 5. Talent profile summary and detailed items (Canonical Phase 4 mapping)
  select
    case
